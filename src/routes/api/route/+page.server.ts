...
export async function POST({ url }) {
  ...
  if (!name || !desc || !recs) return error(400); // invalid param
  const nodes = recs.split(',');
  if (!nodes.every((v) => isUUID(v))) return error(400); // invalid param
  const nodesR = nodes.map((v) => DataBase.getRecord(v as UUID) as PublicRecord);
  if (!nodesR.every((v) => v)) return error(404); // invalid param

  const route = await DataBase.addRoute(name, desc, ...nodesR);
  if (!route) return error(400); // content mod check
