...
export function GET({ params }) {
  ...
  if (!rou) return error(404); // route not found
...
export async function POST({ params, url }) {
  ...
  if (name) {
    ...
    if (!ret) return error(400); // content mod check
  }
  if (desc) {
  ...
  if (isUUID(addId)) {
    ...
    if (addIndex !== undefined) {
      ...
      if (Number.isNaN(index)) return error(400); // invalid param
    }
    ret = await DataBase.addToRoute(rou, addId, index);
    if (!ret) return error(404); // record not found
  }
  if (isUUID(delId)) {
